...
def get_vis_edge_info(r):
    rel_type = get_rel_type(r)
    return({"from": id(r.start_node), "to": id(r.end_node), "label": rel_type})
...
def draw(graph, options, physics=False, limit=100):
    ...
    for row in data:
        ...
        if rel is not None:
            ...
            edges.append({"from": source_info["id"], "to": target_info["id"], "labels": rel["id"]})
